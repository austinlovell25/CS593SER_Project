...
class GraspPlanner(Node):
    ...
    # Quaternion for 180° about x: (x=1, y=0, z=0, w=0)
    ...
    def pick(self, object_id: str, object_pose: Pose) -> bool:
        ...
        # Position the gripper 10 cm above the object, pointing straight down.
        pre_grasp = Pose()
        pre_grasp.position = Point(
            x=object_pose.position.x,
            y=object_pose.position.y,
            z=object_pose.position.z + 0.2   # 20 cm above object centre
        )
        pre_grasp.orientation = self.TOP_DOWN_ORIENTATION
        self.publish_pose_axes(pre_grasp, 'pre_grasp')
        self.get_logger().info(
            f'Pre-grasp target: ({pre_grasp.position.x:.3f}, '
            f'{pre_grasp.position.y:.3f}, {pre_grasp.position.z:.3f})'
        )
        if not self.move_arm_to_pose(pre_grasp):
            return False

        # ---- TODO 3: Descend to grasp ----
        # Allow gripper-object collision atomically with the planning request.
        grasp_pose = copy.deepcopy(pre_grasp)
        grasp_pose.position.z = object_pose.position.z + 0.08  # near top of object
        self.publish_pose_axes(grasp_pose, 'grasp')
        self.get_logger().info(
            f'Grasp target: ({grasp_pose.position.x:.3f}, '
            f'{grasp_pose.position.y:.3f}, {grasp_pose.position.z:.3f})'
        )
        if not self.move_arm_to_pose(grasp_pose, acm_object_id=object_id,
